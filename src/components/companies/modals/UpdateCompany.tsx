...
const UpdateCompany = ({ id, open, setOpen }: UpdateCompanyModalProps) => {
      ...
      if (data.error) {
        toast.custom((t) => (
          <CustomToast
            ...
            type="error"
            title="error"
            message="Couldn't Update Company. Please Try Again!"
          />
        ));
      } else {
        toast.custom((t) => (
  ...
  useEffect(() => {
    if (data) {
      ...
      setWorkingHours(data.working_hours ? data.working_hours : "");
      setLicense(data.license ? data.license : "");
