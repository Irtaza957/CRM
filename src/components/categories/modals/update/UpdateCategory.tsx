...
const UpdateCategory = ({ id, open, setOpen }: UpdateCategoryModalProps) => {
  ...
  useEffect(() => {
    if (data) {
      ...
      setColor(data.color ? data.color : "#000000");
      setDescription(data.description ? data.description : "");
      setCategoryCode(data.code ? data.code : "");
      setCategoryName(data.category_name ? data.category_name : "");
    }
  }, [data]);
        ...
        >
          ...
          <div className="col-span-2 flex w-full flex-col items-center justify-center space-y-2.5">
            ...
            <div className="grid w-full grid-cols-6 gap-6">
              <ImageUploader
                link={data?.icon ? data.icon : ""}
                label="Icon"
                setImage={setIcon}
              />
              <ImageUploader
                link={data?.thumbnail ? data.thumbnail : ""}
                label="Thumbnail"
                setImage={setThumbnail}
              />
              <ImageUploader
                link={data?.cover_image ? data.cover_image : ""}
                label="Cover Image"
